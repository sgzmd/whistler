...
unsigned char RadioSendingMotionCallback::parity(unsigned long ino) {
  int noofones = 0;
...
RadioSendingMotionCallback::RadioSendingMotionCallback(
    unsigned short ce_pin,
    unsigned short csn_pin) {
  this->radio_ = new RF24(ce_pin, csn_pin);
}

void RadioSendingMotionCallback::init() {
...
void RadioSendingMotionCallback::OnMotionDetected(
